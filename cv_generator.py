# cv_generator.py
# -------------------------------------------------------------
# Generador de CV en PDF con Flask + ReportLab
# Mejoras:
# - Formulario con botones "Agregar" para Experiencia, Formación e Habilidades (dinámico con JS)
# - 4 plantillas: "classic", "twocol", "minimal", "modern"
# - Foto por URL y QR opcional (website/LinkedIn)
# -------------------------------------------------------------
...
# QR opcional (silencioso si no está instalado)
try:
    import qrcode
except Exception:
    qrcode = None
...
def collect_skills(data: dict):
    # Puede venir como inputs múltiples "skill" o como cadena separada por comas
...
def build_pdf_twocol(data: dict, accent="#0b7285") -> bytes:
    ...
    if contact_rows:
        ...
        t.setStyle(TableStyle([
            ('VALIGN',(0,0),(-1,-1),'TOP'),('BOTTOMPADDING',(0,0),(-1,-1),2),('LEFTPADDING',(0,0),(-1,-1),0),('RIGHTPADDING',(0,0),(-1,-1),0)
        ]))
        story.append(t); story.append(Spacer(1, 6))
...
def build_pdf_minimal(data: dict) -> bytes:
    ...
    for title, company, dates, desc in collect_experiences(data):
        ...
        for b in lines_to_bullets(desc): story.append(Paragraph(b, styles['ListItem'], bulletText='–'))
    if collect_experiences(data): hr()
...
def build_pdf_modern(data: dict, accent="#2563eb") -> bytes:
    ...
    # Barra superior con nombre/rol
    head = Table([[Paragraph(data.get('full_name') or 'Nombre Apellido', styles['Name']), Paragraph(data.get('role',''), styles['HeaderSmall'])]], colWidths=[None, 6*cm])
    ...
    story.extend([Paragraph('Habilidades', styles['Section'])]) if collect_skills(data) else None
    if collect_skills(data):
        story.append(Paragraph(" · ".join(collect_skills(data)), styles['Body']))
        story.append(Spacer(1,6))
    ...
    if collect_education(data): story.append(Paragraph('Formación', styles['Section']))
    for t_, s_, d_ in collect_education(data):
    ...
    <div class=\"card\">
      ...
      <form method=\"post\" action=\"{{ url_for('generate') }}\" id=\"cvform\">
        ...
        <div class=\"section\">
          ...
          <div class=\"btns\">
            <button type=\"button\" class=\"btn ghost\" onclick=\"addSkill()\">+ Agregar habilidad</button>
          </div>
          <p class=\"small\">Consejo: añade una por línea con el botón.</p>
        </div>
        ...
        <div class=\"btns\">
          <button type=\"button\" class=\"btn ghost\" onclick=\"addExperience()\">+ Agregar experiencia</button>
        </div>
        ...
        <div class=\"btns\">
          <button type=\"button\" class=\"btn ghost\" onclick=\"addEducation()\">+ Agregar formación</button>
        </div>
        ...
        <p class=\"small\">Privacidad: el PDF se genera al vuelo y no se guarda en servidor (ejemplo local).</p>
      </form>
    </div>
  </div>
...
<script>
  ...
  // ======= Precargar demo si viene del servidor =======
  (function preload(){
    const demo = {{ 'true' if data.template in ['classic','twocol','minimal','modern'] and (data.full_name or data.exp_title) else 'false' }};
...
def generate():
    ...
    if tpl == 'twocol':
        pdf = build_pdf_twocol(data)
    elif tpl == 'minimal':
        pdf = build_pdf_minimal(data)
    elif tpl == 'modern':
        pdf = build_pdf_modern(data)
    else:
        pdf = build_pdf_classic(data)
...
if __name__ == '__main__':
    # pip install flask reportlab requests qrcode[pil]
    app.run(debug=True)
